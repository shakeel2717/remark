                        ...
                        <tbody>
                            @foreach ($allCustomers as $customer)
                                <tr>
                                    <td>
                                        <a class="media align-items-center" href="">
                                            <div class="media-body">
                                                <span class="d-block h5 text-hover-primary mb-0"> {{ $loop->iteration }}
                                                </span>
                                            </div>
                                        </a>
                                    </td>
                                    <td>
                                        <span class="d-block h5 mb-0"><a
                                                href="{{ route('customer.show', ['customer' => $customer->id]) }}">{{ $customer->name }}</a></span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <span
                                            class="d-block h5 mb-0">{{ \Carbon\Carbon::createFromTimeStamp(strtotime($customer->created_at))->diffForHumans() }}</span>
                                    </td>
                                    <td>
                                        <span class="d-block h5 mb-0"><button class="btn btn-white"
                                                id="customerbox{{ $loop->iteration }}" data-toggle="modal"
                                                data-target="#customermoda{{ $loop->iteration }}">Detail</button></span>
                                        <!-- Modal -->
                                        <div id="customermoda{{ $loop->iteration }}" class="modal fade" tabindex="-1"
                                            role="dialog" aria-labelledby="customermoda{{ $loop->iteration }}Title"
                                            aria-hidden="true">
